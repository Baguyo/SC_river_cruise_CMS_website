<?php 
        ...
        $get_service_information = $connection->query("SELECT firefly_watching_description, firefly_watching_services FROM service_page_firefly_watching WHERE id = 1");
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST"){
            if(isset($_POST['add_information'])){
                ...
                $update_firefly_watching_service_stmt = $connection->prepare("UPDATE service_page_firefly_watching SET firefly_watching_description = ?, firefly_watching_services = ? WHERE id = 1");
                $update_firefly_watching_service_stmt->bind_param("ss", $description, $services);
                $update_firefly_watching_service_stmt->execute();

                    $update_firefly_watching_service_stmt->store_result();

                    if($update_firefly_watching_service_stmt->affected_rows == 1){
                        $success_update_service = "<p class='text-success'> Firefly watching service information has been successfully updated. <a href='../services.php'> View </a> </p>";
                    }
                    $error = array();
                    if( !empty($_FILES) ){
                        ...
                        foreach ($_FILES['image']['tmp_name'] as $key => $value) {
                            ...
                            if(in_array($ext, $extension)){
                                ...
                                if(!file_exists("../images/$file_name")){
                                    ...
                                    $insert_service_image_stmt = $connection->prepare("INSERT INTO service_page_firefly_watching (firefly_watching_image_name) VALUES (?)");
    ...
    <div class="card-body bg-light">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea autofocus id="my-textarea" class="form-control" name="description" rows="3"><?= $information_row['firefly_watching_description']?></textarea>
            </div>


            <div class="form-group">
                <label for="my-textarea">Service to offer</label>
                <textarea id="my-textarea" class="form-control" name="service" rows="2"><?=  str_replace( array('\r\n', '\n', '\r'), "\r\n", $information_row['firefly_watching_services'] ) ?></textarea>
                <span>
