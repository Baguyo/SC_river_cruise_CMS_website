...
<?php
    $get_all_queries = $connection->query("SELECT * FROM admin_queries");
    ...
    if(isset($_GET['mailsts']) && isset($_GET['id'])){
        ...
        if($change_mail_status == "Yes"){
            $change_status_stmt = $connection->prepare("UPDATE admin_queries SET Mailed = 'No' WHERE id = ?");
        }elseif($change_mail_status == "No"){
            $change_status_stmt = $connection->prepare("UPDATE admin_queries SET Mailed = 'Yes' WHERE id = ?");
        }
                        ...
                        <div class="page-header">
                            <div class="card">
                                <div class="card-header bg-warning text-white">
                                    <h1 class="text-center">SC QUERIES </h1>
                                </div>
                                <div class="card-body">
                                    ...
                                    <table class="table table-bordered table-hover">
                                       ...
                                       <tbody>
                                           <?php
                                                ...
                                                while($row = $get_all_queries->fetch_assoc()){
                                                    ...
                                                    echo "<tr>";
                                                        ...
                                                        echo "<td> ". $mailed . " <div> <a href='queries.php?mailsts={$row['Mailed']}&id={$row['id']}' class=''>Change</a> </div> </td>";
                                                        echo "<td> <a href='#' dlt='$row[id]' class='btn btn-danger delete btn-sm'>DELETE</a> </td>";
                                                        echo "<td> <a href='queries_mail.php?id={$row['id']}' class='btn btn-success btn-sm'>MAIL</a> </td>";
...
<?php 
      if(isset($_POST['dlt'])){
        ...
        $delete_reservation_stmt = $connection->prepare("DELETE FROM admin_queries WHERE id = ?");
